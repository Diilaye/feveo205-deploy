...
export const getArrondissements = (regionCode, departementCode) => {
    ...
    const departementEntry = Object.values(region.departements).find((dept) => dept.code === departementCode);
    if (!departementEntry)
        return [];
    return departementEntry.arrondissements.map(arr => ({
        code: arr.code,
        nom: arr.nom
    }));
};
...
export const getCommunes = (regionCode, departementCode, arrondissementCode) => {
    ...
    if (!departementEntry)
        return [];
    const arrondissement = departementEntry.arrondissements.find(arr => arr.code === arrondissementCode);
    if (!arrondissement)
        return [];
...
export const validateGeographicLocation = (region, departement, arrondissement, commune) => {
    ...
    if (!arrondissementData)
        return false;
    return arrondissementData.communes.some(c => c.code === commune);
};
//# sourceMappingURL=senegalGeography.js.map