...
const RegionDepartementReport = ({ authToken, apiBaseUrl }) => {
        ...
        try {
            ...
            if (response?.data?.success) {
                ...
                for (const regionCode in data) {
                    ...
                    for (const deptCode in data[regionCode]) {
                        const deptGiesCount = data[regionCode][deptCode];
                        // Filter GIEs for this department
                        const departementGIEs = gies.filter(gie => gie.region === regionCode && gie.departement === deptCode);
                        const departementData = {
                            nom: deptCode, // Use code as name if real name not available
                            code: deptCode,
                            giesCount: deptGiesCount,
                            gies: departementGIEs
                        };
                        regionGIEs.departements.push(departementData);
                        regionGIEs.totalGIEs += deptGiesCount;
                    }
    ...
    const renderContent = () => {
        ...
        if (regionsData.length === 0)
            return renderEmpty();
        return (_jsxs("div", { className: "p-5", children: [renderStats(), _jsx("div", { className: "space-y-4", children: regionsData.map(renderRegion) })] }));
    };
...
//# sourceMappingURL=RegionDepartementGIEReport.js.map